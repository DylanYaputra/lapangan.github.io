...
function getBookingHistory($conn, $user_id) {
    ...
    // Periksa tabel futsal_booking
    $check_futsal = $conn->query("SHOW TABLES LIKE 'futsal_booking'");
    if ($check_futsal->num_rows > 0) {
        // Cek apakah kolom user_id ada di tabel futsal_booking
        $check_column = $conn->query("SHOW COLUMNS FROM futsal_booking LIKE 'user_id'");
        $has_user_id = $check_column->num_rows > 0;
        
        // Ambil booking dari tabel futsal_booking berdasarkan email atau user_id
        if ($has_user_id) {
            $futsal_query = "SELECT b.*, 'futsal' as field_type ";
            
            // Tambahkan join dan price_per_hour jika tabel fields dan join clause tersedia
            if (!empty($join_clause)) {
                $futsal_query .= ", $field_column ";
            } else {
                $futsal_query .= ", 0 as price_per_hour ";
            }
            
            $futsal_query .= " FROM futsal_booking b ";
            
            // Tambahkan join clause jika tersedia
            if (!empty($join_clause)) {
                $futsal_query .= " $join_clause ";
            }
            
            $futsal_query .= " WHERE b.customer_email = ? OR b.user_id = ?
                             ORDER BY b.booking_date DESC, b.booking_time DESC";
            
            $stmt = $conn->prepare($futsal_query);
            ...
            $futsal_query .= " WHERE b.customer_email = ?
                             ORDER BY b.booking_date DESC, b.booking_time DESC";
        ...
        $stmt->execute();
        $futsal_result = $stmt->get_result();
        
        if ($futsal_result && $futsal_result->num_rows > 0) {
            while ($row = $futsal_result->fetch_assoc()) {
                $booking_history[] = $row;
            }
        }
    }
    ...
    // Periksa tabel tennis_booking
    $check_tennis = $conn->query("SHOW TABLES LIKE 'tennis_booking'");
    if ($check_tennis->num_rows > 0) {
        // Cek apakah kolom user_id ada di tabel tennis_booking
        $check_column = $conn->query("SHOW COLUMNS FROM tennis_booking LIKE 'user_id'");
        $has_user_id = $check_column->num_rows > 0;
        
        // Ambil booking dari tabel tennis_booking
        if ($has_user_id) {
            $tennis_query = "SELECT b.*, 'tennis' as field_type ";
            
            // Tambahkan join dan price_per_hour jika tabel fields dan join clause tersedia
            if (!empty($join_clause)) {
                $tennis_query .= ", $field_column ";
            } else {
                $tennis_query .= ", 0 as price_per_hour ";
            }
            
            $tennis_query .= " FROM tennis_booking b ";
            
            // Tambahkan join clause jika tersedia
            if (!empty($join_clause)) {
                $tennis_query .= " $join_clause ";
            }
            
            $tennis_query .= " WHERE b.customer_email = ? OR b.user_id = ?
                            ORDER BY b.booking_date DESC, b.booking_time DESC";
            
            $stmt = $conn->prepare($tennis_query);
            $stmt->bind_param("si", $user_email, $user_id);
        } else {
            $tennis_query = "SELECT b.*, 'tennis' as field_type ";
            ...
            $tennis_query .= " WHERE b.customer_email = ?
                            ORDER BY b.booking_date DESC, b.booking_time DESC";
        ...
        $stmt->execute();
        $tennis_result = $stmt->get_result();
        
        if ($tennis_result && $tennis_result->num_rows > 0) {
            while ($row = $tennis_result->fetch_assoc()) {
                $booking_history[] = $row;
            }
        }
    }
