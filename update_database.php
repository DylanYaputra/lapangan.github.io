...
if ($check_fields->num_rows == 0) {
    ...
    $create_fields = "CREATE TABLE IF NOT EXISTS fields (
        ...
        field_type ENUM('futsal', 'badminton', 'tennis') NOT NULL,
...
// Cek dan buat tabel futsal_booking jika belum ada
$check_futsal = $conn->query("SHOW TABLES LIKE 'futsal_booking'");
if ($check_futsal->num_rows == 0) {
    $result = createBookingTable($conn, 'futsal_booking');
    if ($result) {
        $messages[] = "Tabel futsal_booking berhasil dibuat.";
    } else {
        $messages[] = "Error membuat tabel futsal_booking: " . $conn->error;
        $success = false;
    }
} else {
    // Tambahkan kolom user_id jika belum ada
    if (addUserIdColumn($conn, 'futsal_booking')) {
        $messages[] = "Tabel futsal_booking diperbarui.";
    } else {
        $messages[] = "Error memperbarui tabel futsal_booking: " . $conn->error;
        $success = false;
    }
}
...
// Cek dan buat tabel tennis_booking jika belum ada
$check_tennis = $conn->query("SHOW TABLES LIKE 'tennis_booking'");
if ($check_tennis->num_rows == 0) {
    $result = createBookingTable($conn, 'tennis_booking');
    if ($result) {
        $messages[] = "Tabel tennis_booking berhasil dibuat.";
    } else {
        $messages[] = "Error membuat tabel tennis_booking: " . $conn->error;
        $success = false;
    }
} else {
    // Tambahkan kolom user_id jika belum ada
    if (addUserIdColumn($conn, 'tennis_booking')) {
        $messages[] = "Tabel tennis_booking diperbarui.";
    } else {
        $messages[] = "Error memperbarui tabel tennis_booking: " . $conn->error;
        $success = false;
    }
}
