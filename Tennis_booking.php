...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $sql = "INSERT INTO tennis_booking (booking_date, booking_time, field, customer_name, customer_phone, customer_email, team_name, notes, status, user_id)
...
$booked_query = "SELECT booking_date, booking_time, field FROM tennis_booking 
...
<html lang="id">
  ...
  <nav class="navbar navbar-expand-lg sticky-top">
    ...
    <form action="booking_process_tennis.php" method="POST" id="bookingForm">
              ...
              <div class="card-body p-4">
                ...
                <div class="time-slots">
                  <?php
                    ...
                    $booked_times = [];
                    foreach ($booked_slots as $field_name => $times) {
                    ...
                    foreach ($time_slots_list as $start_time) {
          ...
          <div class="col-md-6 mb-4">
            <div class="custom-card shadow-sm h-100">
              ...
              <div class="card-body p-4">
                <div class="row">
                  <?php
                    // Query untuk mengambil data lapangan tennis dari database
                    $get_fields_query = "SELECT * FROM fields WHERE field_type = 'tennis'";
                    $fields_result = $conn->query($get_fields_query);
                    $fields_from_db = [];
                    
                    // Jika berhasil mengambil data dari database
                    if ($fields_result && $fields_result->num_rows > 0) {
                        while ($row = $fields_result->fetch_assoc()) {
                            $fields_from_db[] = [
                                'id' => $row['id'],
                                'name' => $row['field_name'],
                                'type' => 'Lapangan indoor',
                    ...
                    $selected_field = isset($_GET['field']) ? $_GET['field'] : ($fields_from_db[0]['value'] ?? '');
                    
                    foreach ($fields_from_db as $field) {
                      $is_available = $field['available'];
                      $disabled = !$is_available ? 'disabled' : '';
                      $checked = ($selected_field == $field['value']) ? 'checked' : '';
                  ?>
