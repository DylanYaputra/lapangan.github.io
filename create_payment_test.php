...
// Cek status booking di futsal_booking
$futsal_query = "SELECT * FROM futsal_booking ORDER BY id DESC";
$futsal_result = $conn->query($futsal_query);

echo '<h3>Futsal Booking</h3>';
if ($futsal_result && $futsal_result->num_rows > 0) {
        ...
        echo '<tr>
                ...
                <td>
                    <form method="post" action="change_status.php" style="display:inline;">
                        ...
                        <input type="hidden" name="booking_table" value="futsal_booking">
    ...
    echo '<div class="alert alert-info">Tidak ada data booking futsal.</div>';
}
...
// Cek status booking di tennis_booking
$tennis_query = "SELECT * FROM tennis_booking ORDER BY id DESC";
$tennis_result = $conn->query($tennis_query);

echo '<h3 class="mt-4">Tennis Booking</h3>';
if ($tennis_result && $tennis_result->num_rows > 0) {
    ...
    while ($row = $tennis_result->fetch_assoc()) {
        echo '<tr>
                ...
                <td>
                    <form method="post" action="change_status.php" style="display:inline;">
                        <input type="hidden" name="booking_id" value="' . $row['id'] . '">
                        <input type="hidden" name="booking_table" value="tennis_booking">
                        <input type="hidden" name="new_status" value="pending_confirmation">
                        <button type="submit" class="btn btn-sm btn-warning">Set Pending Confirmation</button>
                    </form>
                </td>
    ...
    echo '<div class="alert alert-info">Tidak ada data booking tennis.</div>';
}
