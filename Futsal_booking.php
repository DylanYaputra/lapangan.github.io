...
// Fungsi untuk cek kolom nama di tabel fields
function checkFieldColumn($conn) {
    $result = $conn->query("SHOW COLUMNS FROM fields LIKE 'field_name'");
    return ($result->num_rows > 0) ? 'field_name' : 'name';
}

// Fungsi untuk cek kolom type di tabel fields
function checkTypeColumn($conn) {
    $result = $conn->query("SHOW COLUMNS FROM fields LIKE 'field_type'");
    return ($result->num_rows > 0) ? 'field_type' : 'type';
}

// Ambil nama kolom yang benar
$field_name_column = checkFieldColumn($conn);
$field_type_column = checkTypeColumn($conn);

// Ambil semua lapangan futsal dari database
$query = "SELECT * FROM fields WHERE $field_type_column = 'futsal'";
...
// Ambil daftar booking yang ada untuk tanggal yang dipilih
$booked_slots = [];
$booked_query = "SELECT booking_date, booking_time, field FROM futsal_booking 
...
if ($stmt) {
    ...
    if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $field = $row['field'];
            if (!isset($booked_slots[$field])) {
                $booked_slots[$field] = [];
            }
            $booked_slots[$field][] = $time;
        }
    }
    $stmt->close();
}
...
// Daftar lapangan dari database
$field_options = [];
foreach ($fields as $field) {
    $name = isset($field[$field_name_column]) ? $field[$field_name_column] : $field['name'];
    $field_options[$name] = [
...
// Field yang dipilih (default: lapangan pertama)
$selected_field = isset($_GET['field']) ? $_GET['field'] : (count($field_options) > 0 ? array_keys($field_options)[0] : '');
?>
<!DOCTYPE html>
<html lang="id">
  <head>
  ...
  <nav class="navbar navbar-expand-lg sticky-top">
            ...
            <div class="custom-card shadow-sm h-100">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="time-slots">
                        <?php
                        // Pengecekan slot yang sudah dipesan untuk lapangan yang dipilih
                        $booked_times = isset($booked_slots[$selected_field]) ? $booked_slots[$selected_field] : [];
                        
                        // Ambil harga lapangan yang dipilih
                        $price = isset($field_options[$selected_field]) ? $field_options[$selected_field]['price'] : 120000;
                        ...
                        foreach ($time_slots as $start_time) {
                            ...
                            $is_booked = in_array($start_time, $booked_times);
                        ...
                        <div class="time-slot p-3 mb-3 border rounded position-relative <?php echo $status_class; ?>">
                            <div class="row">
                                <div class="col-md-7">
                                    <div class="form-check">
                                        <input 
                                            ...
                                            type="radio" 
                                            name="bookingTime" 
                                            id="time<?php echo str_replace(':', '', $start_time); ?>" 
                                            value="<?php echo $start_time; ?>" 
                                            <?php echo $disabled; ?>
                                        >
                                        <label class="form-check-label" for="time<?php echo str_replace(':', '', $start_time); ?>">
                                            <?php echo $time_range; ?>
                                        </label>
                                    </div>
                                ...
                                <div class="col-md-5 text-end">
                                    <span class="badge <?php echo $badge_class; ?> p-2">Rp <?php echo number_format($price, 0, ',', '.'); ?></span>
                                </div>
                            </div>
                        </div>
                        <?php } ?>
          ...
          <div class="col-md-6 mb-4">
            <div class="custom-card shadow-sm h-100">
                ...
                <div class="card-body p-4">
                    <div class="row">
                        <?php 
                        // Tampilkan semua lapangan futsal dari database
                        foreach ($fields as $field):
                            $field_name = isset($field[$field_name_column]) ? $field[$field_name_column] : $field['name'];
                            $is_available = ($field['status'] ?? 'available') == 'available';
                            $disabled = !$is_available ? 'disabled' : '';
                            $checked = ($selected_field == $field_name) ? 'checked' : '';
                        ?>
                        <div class="col-md-6 mb-3">
                            <div class="field-option p-3 border rounded h-100 <?php echo !$is_available ? 'bg-light' : ''; ?>">
                                <h5><?php echo $field_name; ?></h5>
                                <p class="small text-muted mb-2">Lapangan indoor</p>
                                <div class="form-check">
                                    <input 
    ...
    // Script untuk menangani perubahan pilihan lapangan
    document.addEventListener('DOMContentLoaded', function() {
        const dateInput = document.getElementById('bookingDate');
        const fieldRadios = document.querySelectorAll('.field-radio');
        
        // Fungsi untuk update URL dengan parameter field dan date
        function updateURL(field, date) {
        ...
        dateInput.addEventListener('change', function() {
            const selectedField = document.querySelector('input[name="fieldChoice"]:checked');
            const fieldValue = selectedField ? selectedField.value : '';
            updateURL(fieldValue, this.value);
        });
        
        // Event listener untuk perubahan lapangan
        fieldRadios.forEach(radio => {
            radio.addEventListener('change', function() {
                updateURL(this.value, dateInput.value);
            });
        });
    });
    </script>
