...
// Harga dan biaya
$field_price = 100000; // Default price if query fails
$admin_fee = 2500;

// Ambil harga dari database berdasarkan nama lapangan
$query_price = "SELECT price_per_hour FROM fields WHERE field_name = ?";
$stmt_price = $conn->prepare($query_price);
if ($stmt_price && !empty($field_name)) {
...
if (isset($_POST['submit_booking']) || isset($_SESSION['booking_data'])) {
    ...
    // Cek kembali harga dari database dengan field_name yang diupdate
    $stmt_price = $conn->prepare($query_price);
    if ($stmt_price) {
        $stmt_price->bind_param("s", $field_name);
        $stmt_price->execute();
...
<nav class="navbar navbar-expand-lg sticky-top">
          ...
          <div class="card-body p-4">
            ...
            <div class="bank-details mb-4">
              ...
              <div class="row mb-3">
                ...
                <div class="col-md-8">PT SportField Indonesia</div>
              </div>
              <div class="row mb-3">
                <div class="col-md-4 text-muted">Nomor Rekening</div>
                <div class="col-md-8 d-flex align-items-center">
                  <span class="fw-bold me-2" id="rekening">12345678901</span>
            ...
            <div class="mb-4">
              ...
              <ul class="payment-steps">
                ...
                <li>Masukkan nomor rekening <strong>12345678901</strong> (PT SportField Indonesia)</li>
            ...
            <div class="mb-4">
              ...
              <form id="paymentConfirmForm" action="upload_payment_proof.php" method="POST" enctype="multipart/form-data">
                ...
                <input type="hidden" name="booking_type" value="futsal">
        ...
        <div class="custom-card shadow-sm">
          ...
          <div class="card-body p-4">
            ...
            <div class="alert alert-info mt-4 small" role="alert">
              ...
              Detail booking juga telah dikirimkan ke email Anda.
            </div>
          </div>
        </div>
      </div>
  ...
  <script>
      ...
      const timer = setInterval(() => {
        if (seconds === 0) {
          if (minutes === 0) {
            ...
            window.location.href = 'futsal_booking.php';
            return;
          }
          minutes--;
          seconds = 59;
    ...
    window.onload = function() {
      // Di sini kita bisa mengambil data dari session PHP menggunakan AJAX
      // atau langsung dari parameter URL jika dikirimkan
      startCountdown();
      
      // Populasikan informasi booking dari PHP session
      // (ini hanya contoh, pada implementasi sebenarnya akan mengambil dari backend)
      // fetchBookingDetails();
    }

    // Contoh fungsi untuk mengambil detail booking dari backend
    function fetchBookingDetails() {
      fetch('get_booking_details.php')
        .then(response => response.json())
        .then(data => {
          document.getElementById('bookingNumber').textContent = data.id;
          document.getElementById('fieldName').textContent = data.field_name;
          document.getElementById('bookingDateTime').textContent = `${data.booking_date}, ${data.booking_time}`;
          document.getElementById('customerName').textContent = data.customer_name;
          document.getElementById('totalPayment').textContent = data.total_price;
          document.getElementById('nominal').textContent = data.total_price;
        })
